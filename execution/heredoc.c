...
int	ft_heredoc_v2(t_node *node, t_mini *data, int n_std_out_hr, g_built *built)
{
	node->ptr = readline("heredoc> ");
	node->difirent = ft_strncmp_exe(node->ptr, node->heredoc_limit);
	if(node->ptr != NULL)
	{
		ft_expand(data, &node->ptr2, node->ptr, built);
		free(node->ptr);
	}
	if (!node->difirent)
		return (1);
...
void	ft_heredoc_3(t_node *node, t_mini *data, g_built *built)
{
	...
	while (data[node->loop].limiter[++node->a] != NULL)
	{
		...
		while (true)
			if (ft_heredoc_v2(node, data, node->n_std_out_hr, built))
				break ;
	}
}
...
void	ft_heredoc_1(t_node *node, t_mini *data, g_built *built)
{
	...
	while (data[node->loop].limiter[++node->a] != NULL)
	{
		node->heredoc_limit = data[node->loop].limiter[node->a];
		while (true)
			if (ft_heredoc_v2(node, data, 1, built))
				break ;
	}
}
